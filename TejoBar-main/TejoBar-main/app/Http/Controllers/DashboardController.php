...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Verificamos que el rol en la sesión coincida con el de la base de datos
        $this->sincronizarRol($usuario);
        $rol = Session::get('rol'); // Actualizamos el rol después de verificar
        ...
        // Dependiendo del rol, mostramos diferentes datos
        switch ($rol) {
            case 'admin':
    ...
    private function getCapitanData($idPersona)
    {
        ...
        // Buscamos los torneos y partidos del equipo del capitán
        ...
        return [
            ...
            'partidosEquipo' => collect(), // Aquí se puede agregar más lógica después
    ...
    public function historial()
    {
        ...
        // Dependiendo del rol, mostramos diferentes datos
        if ($rol === 'admin') {
            // El admin puede ver todos los apartados de todos los usuarios
            ...
            // Los jugadores solo ven sus propios apartados
    ...
    private function sincronizarRol($usuario)
    {
        ...
        // Si es jugador o capitán, verificamos si realmente es capitán de algún equipo
        if (($rolSesion === 'jugador' || $rolSesion === 'capitan') && $usuario->jugador) {
            ...
            if ($esCapitanReal && $rolSesion !== 'capitan') {
                // Lo cambiamos a capitán en la base de datos y en la sesión
                $usuario->update(['rol' => 'capitan']);
                Session::put('rol', 'capitan');
            } elseif (!$esCapitanReal && $rolSesion === 'capitan') {
                // Lo cambiamos a jugador en la base de datos y en la sesión
                $usuario->update(['rol' => 'jugador']);
                Session::put('rol', 'jugador');
            }
        }

        // Si el rol en la base de datos es diferente al de la sesión, los sincronizamos
