...
class Cancha(models.Model):
    ...
    disponibilidad = models.CharField(
        ...
        unique=True,
        error_messages={
            "blank": "El nombre de la cancha es obligatorio.",
            "null": "El nombre de la cancha es obligatorio.",
            "unique": "Ya existe una cancha con este nombre.",
        },
        validators=[
            MinLengthValidator(
                2,
                message="El nombre de la cancha debe tener entre 2 y 50 caracteres.",
            ),
            MaxLengthValidator(
                50,
                message="El nombre de la cancha debe tener entre 2 y 50 caracteres.",
    ...
    precio_por_hora = models.FloatField(
        default=8000.0,
        validators=[
            MinValueValidator(0.01, message="La tarifa por hora debe ser mayor a 0.")
        ],
        error_messages={
            "invalid": "La tarifa por hora debe ser un valor numérico.",
        }
    )
    descripcion = models.TextField(
        ...
        null=True,
        validators=[
            MaxLengthValidator(200, message="La descripción no puede superar los 200 caracteres.")
        ]
    )
    ...
    def clean(self):
        super().clean()
        nombre = (self.disponibilidad or "").strip()
        if not nombre:
            return
        ...
        if qs.exists():
            raise ValidationError({"disponibilidad": "Ya existe una cancha con este nombre."})

    def save(self, *args, **kwargs):
        self.full_clean()
        super().save(*args, **kwargs)

    def partidos_futuros_programados(self):
        """Partidos pendientes o confirmados con fecha/hora aún no cumplida."""
        from django.utils import timezone

        hoy = timezone.localdate()
        ahora = timezone.localtime().strftime("%H:%M")
        candidatos = self.partidos.exclude(estado=Partido.ESTADO_CANCELADA).filter(
            estado__in=[Partido.ESTADO_PENDIENTE, Partido.ESTADO_CONFIRMADA],
        )
        ids_futuros = list(candidatos.filter(fecha__gt=hoy).values_list("pk", flat=True))
        for partido in candidatos.filter(fecha=hoy):
            hora = (partido.hora or "00:00").strip().zfill(5)[:5]
            if hora >= ahora[:5]:
                ids_futuros.append(partido.pk)
        return Partido.objects.filter(pk__in=ids_futuros).order_by("fecha", "hora")

    @property
    def can_be_deleted(self) -> bool:
        return not self.partidos_futuros_programados().exists()
...
class Partido(models.Model):
    ...
    @property
    def clave_orden_fecha_hora(self) -> tuple:
        return (self.fecha, (self.hora or "00:00").strip().zfill(5)[:5])

    @classmethod
    def ordenar_por_fecha_hora(cls, partidos, descendente: bool = False):
        return sorted(partidos, key=lambda p: p.clave_orden_fecha_hora, reverse=descendente)
